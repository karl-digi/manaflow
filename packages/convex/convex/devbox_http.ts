...
function extractNetworkingUrls(
  httpServices: Array<{ port: number; url: string }>
) {
...
// Type-safe references to devboxInstances functions (will be generated by convex dev)
// eslint-disable-next-line @typescript-eslint/no-explicit-any
const devboxApi = (api as any).devboxInstances as {
  create: FunctionReference<"mutation", "public">;
  list: FunctionReference<"query", "public">;
  getByMorphId: FunctionReference<"query", "public">;
...
export const createInstance = httpAction(async (ctx, req) => {
  ...
  try {
    ...
    const morphData = (await morphResponse.json()) as {
      ...
      status: string;
      networking?: {
        http_services?: Array<{ port: number; url: string }>;
      };
    };
...
async function handleGetInstance(
  ...
  try {
    ...
    const instance = await ctx.runQuery(devboxApi.getByMorphId, {
      teamSlugOrId,
      morphInstanceId,
    });
    ...
    const morphResponse = await morphFetch(`/instance/${morphInstanceId}`);
    ...
    return jsonResponse({
      ...instance,
      status,
      vscodeUrl: vscodeUrl ?? instance.vscodeUrl,
      workerUrl: workerUrl ?? instance.workerUrl,
      vncUrl: vncUrl ?? instance.vncUrl,
    });
  } catch (error) {
...
async function handlePauseInstance(
  ...
  try {
    ...
    // Pause via Morph API
    const morphResponse = await morphFetch(
      `/instance/${morphInstanceId}/pause`,
      {
        method: "POST",
      }
    );
...
async function handleResumeInstance(
  ...
  try {
    ...
    const morphResponse = await morphFetch(
      `/instance/${morphInstanceId}/resume`,
      {
        method: "POST",
      }
    );
...
async function handleGetInstanceSsh(
  ...
  try {
    ...
    const morphResponse = await morphFetch(
      `/instance/${morphInstanceId}/ssh/key`
    );
    ...
    return jsonResponse({
      morphInstanceId,
      sshCommand: `ssh ${sshData.access_token}@ssh.cloud.morph.so`,
...
export const instanceActionRouter = httpAction(async (ctx, req) => {
  ...
  const morphInstanceId = pathParts[pathParts.length - 2];
  ...
  if (path.endsWith("/exec")) {
    ...
    return handlePauseInstance(ctx, morphInstanceId, body.teamSlugOrId);
  } else if (path.endsWith("/resume")) {
    return handleResumeInstance(ctx, morphInstanceId, body.teamSlugOrId);
  } else if (path.endsWith("/stop")) {
    return handleStopInstance(ctx, morphInstanceId, body.teamSlugOrId);
  }
...
export const instanceGetRouter = httpAction(async (ctx, req) => {
  ...
  if (path.endsWith("/ssh")) {
    const pathParts = path.split("/");
    const morphInstanceId = pathParts[pathParts.length - 2];
    return handleGetInstanceSsh(ctx, morphInstanceId, teamSlugOrId);
  }

  // Default: get instance details
  const pathParts = path.split("/");
  const morphInstanceId = pathParts[pathParts.length - 1];
  return handleGetInstance(ctx, morphInstanceId, teamSlugOrId);
});

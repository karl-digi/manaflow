...
const AgentNeedsInputRequestSchema = z.object({
  taskRunId: typedZid("taskRuns"),
...
/**
 * HTTP endpoint called when an agent needs user input.
 * This should NOT mark the run as completed.
 */
export const agentNeedsInput = httpAction(async (ctx, req) => {
  const auth = await getWorkerAuth(req, {
    loggerPrefix: "[convex.notifications]",
  });
  if (!auth) {
    console.error("[convex.notifications] Auth failed for agent-needs-input");
    return jsonResponse({ code: 401, message: "Unauthorized" }, 401);
  }
  ...
  const validation = AgentNeedsInputRequestSchema.safeParse(json);
  if (!validation.success) {
    console.warn(
      "[convex.notifications] Invalid agent-needs-input payload",
      validation.error
    );
    return jsonResponse({ code: 400, message: "Invalid input" }, 400);
  }
  ...
  await ctx.runMutation(
    internal.notifications_http.createAgentNeedsInputNotification,
    {
      taskRunId,
      taskId: taskRun.taskId,
      teamId: taskRun.teamId,
  ...
  console.log("[convex.notifications] Created agent-needs-input notification", {
    taskRunId,
    taskId: taskRun.taskId,
  });
...
export const createAgentStoppedNotification = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    // Check if notification already exists for this run + type (avoid duplicates)
    const existingNotifications = await ctx.db
      ...
      .filter((q) => q.eq(q.field("type"), notificationType))
      .first();
...
/**
 * Internal mutation to create an agent-needs-input notification.
 */
export const createAgentNeedsInputNotification = internalMutation({
  args: {
    taskRunId: v.id("taskRuns"),
    taskId: v.id("tasks"),
    teamId: v.string(),
  ...
  handler: async (ctx, args) => {
    const now = Date.now();
    const notificationType = "run_needs_input";

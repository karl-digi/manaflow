...
/**
 * Posts an initial "in progress" comment to GitHub PR immediately when workspace starts.
 * This comment contains just the links (workspace, dev browser, diff heatmap) and indicates
 * that screenshots are being captured. The comment will be edited later with full content.
 */
export const postInitialPreviewComment = internalAction({
  ...
  handler: async (ctx, args): Promise<{ ok: true; commentId: number; commentUrl: string } | { ok: false; error: string }> => {
    ...
    try {
      ...
      // Build initial comment with links and "in progress" status
      ...
      commentSections.push(
        ...
        "",
        "_This comment will be updated automatically when screenshots are ready._",
      );
      ...
      // Update preview run with comment info
...
/**
 * Updates an existing GitHub comment with full screenshot content.
 * Used when we've already posted an initial "in progress" comment and now have screenshots ready.
 */
export const updatePreviewComment = internalAction({
  ...
  handler: async (ctx, args): Promise<{ ok: true; commentUrl?: string } | { ok: false; error: string }> => {
    ...
    try {
      ...
      if (!accessToken) {
        console.error(
          "[github_pr_comments] Failed to get access token for updating preview comment",
          { installationId },
        );
        return { ok: false, error: "Failed to get access token" };
      }
      ...
      // Build comment sections (same logic as postPreviewComment)
      const commentSections: string[] = ["## Preview Screenshots"];

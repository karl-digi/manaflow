...
const appendPreviewSource = (rawUrl: string): string => {
  try {
    const parsed = new URL(rawUrl);
    parsed.searchParams.set("src", "preview.new");
    return parsed.toString();
  } catch {
    return rawUrl.includes("?")
      ? `${rawUrl}&src=preview.new`
      : `${rawUrl}?src=preview.new`;
  }
};

const findDevServerUrl = ({
  networking,
  preferredPorts,
}: {
  networking?: TaskRunDoc["networking"];
  preferredPorts?: readonly number[] | null;
}): string | null => {
  if (!networking || networking.length === 0) {
    return null;
  }
  ...
  const safePreferred = (preferredPorts ?? []).filter(
    (port) => !RESERVED_PREVIEW_PORTS.has(port),
  );
  if (safePreferred.length > 0) {
    const preferredMatch = runningServices.find((service) =>
      safePreferred.includes(service.port),
    );
  ...
  const nonReserved = runningServices.find(
    (service) => !RESERVED_PREVIEW_PORTS.has(service.port),
  );
  if (nonReserved?.url) {
    return nonReserved.url;
  }
...
const buildLinksRow = ({
  ...
  if (workspaceUrl) {
    linkParts.push(
      `[Open Workspace (1 hr)](${appendPreviewSource(workspaceUrl)})`,
    );
  }

  if (devServerUrl) {
    linkParts.push(
      `[Open Dev Browser (1 hr)](${appendPreviewSource(devServerUrl)})`,
    );
  }

  linkParts.push(
    `[Open Diff Heatmap](${appendPreviewSource(
      `https://0github.com/${repoFullName}/pull/${prNumber}`,
    )})`,
  );
...
export const postPreviewComment = internalAction({
  ...
  handler: async (ctx, args) => {
    ...
    try {
      ...
      const previewConfig = previewRunPayload?.config;
      if (!previewRun || !previewConfig) {
      ...
      const environment = previewConfig.environmentId
        ? await ctx.runQuery(internal.environments.getByIdInternal, {
            id: previewConfig.environmentId,
          })
        : null;

      const taskRun = previewRun.taskRunId
        ? await ctx.runQuery(internal.taskRuns.getById, {
            id: previewRun.taskRunId,
          })
        : null;

      const workspaceUrl =
        taskRun?.vscode?.workspaceUrl ?? taskRun?.vscode?.url ?? null;

      const devServerUrl = findDevServerUrl({
        networking: taskRun?.networking,
        preferredPorts: environment?.exposedPorts,
      });

      const linksRow = buildLinksRow({
        repoFullName,
        prNumber,
        workspaceUrl,
        devServerUrl,
      ...
      const headingSection = linksRow
        ? ["## Preview Screenshots", linksRow].join("\n\n")
        : "## Preview Screenshots";

      const commentSections: string[] = [headingSection, latestSection];

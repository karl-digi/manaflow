# Daytona snapshot Dockerfile for cmux devbox template (Docker-enabled).
#
# NOTE: This file intentionally lives at the package root because @daytonaio/sdk
# (Image.fromDockerfile) uses the Dockerfile's directory as the build context.
# The existing template Dockerfiles live under template/ and rely on a build
# context rooted at packages/cmux-devbox-2, which does not work with the SDK.
#
# This image preinstalls:
# - cmux-code (VS Code server)
# - TigerVNC + noVNC (browser desktop)
# - Chrome (for computer automation)
# - Docker (dockerd + CLI)
# - Go worker daemon + VNC auth proxy
...
# Configure SSH server on port 10000 (since 22 may not be exposed by E2B/Daytona)
...
RUN ARCH=$(dpkg --print-architecture) && \
    ...
    RELEASE_URL=$(curl -fsSL https://api.github.com/repos/manaflow-ai/vscode-1/releases/latest | jq -r "[.assets[].browser_download_url | select(test(\"vscode-server-linux-${ARCH}-web.tar.gz$\"))][0]") && \
    if [ "$RELEASE_URL" = "null" ] || [ -z "$RELEASE_URL" ]; then echo "cmux-code release not found for arch=${ARCH}" >&2; exit 1; fi && \
...
# Create user account (template expects a 'user' account)
...
# Expose ports (Daytona handles exposure, proxy handles auth/warnings)

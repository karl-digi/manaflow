...
export const getApiScriptsByScriptNameQueryKey = (options: Options<GetApiScriptsByScriptNameData>) => createQueryKey('getApiScriptsByScriptName', options);

/**
 * Get a script by name
 * Fetches a TypeScript script that can be executed remotely by workers
 */
export const getApiScriptsByScriptNameOptions = (options: Options<GetApiScriptsByScriptNameData>) => {
    return queryOptions({
        queryFn: async ({ queryKey, signal }) => {
            const { data } = await getApiScriptsByScriptName({
                ...options,
                ...queryKey[0],
                signal,
                throwOnError: true
        ...
        queryKey: getApiScriptsByScriptNameQueryKey(options)
    });
};
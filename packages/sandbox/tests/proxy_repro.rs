...
async fn proxy_tls_handshake_should_not_panic() {
    ...
    // 3. Send CONNECT
    stream.write_all(b"CONNECT google.com:443 HTTP/1.1\r\nHost: google.com:443\r\n\r\n").await.unwrap();
    
    let mut buf = [0u8; 1024];
    let n = stream.read(&mut buf).await.unwrap();
    let response = String::from_utf8_lossy(&buf[..n]);
    assert!(response.contains("200 Connection Established"), "Proxy did not accept CONNECT");
    ...
    match child.try_wait() {
        Ok(Some(status)) => {
            panic!("Proxy process exited unexpectedly (likely panicked): {}", status);
        }
        Ok(None) => {

...
async def test_git_diff(vmid: int, cf_domain: str, repo_root: Path) -> None:
    """Test the git-diff upload approach."""
    client = PveExecClient(cf_domain)
    remote_repo_root = "/tmp/cmux-test"

    print(f"\n=== Testing git-diff upload for container {vmid} ===\n")
    ...
    exit_code, stdout, stderr = client.http_exec(vmid, "echo 'HTTP exec works!'")
    if exit_code != 0:
    ...
    for attempt in range(1, max_attempts + 1):
        ...
        exit_code, stdout, stderr = client.http_exec(vmid, clone_cmd, timeout=300)
    ...
    if patch_path is None:
        ...
        exit_code, stdout, stderr = client.http_exec(vmid, upload_cmd, timeout=120)
        ...
        exit_code, stdout, stderr = client.http_exec(vmid, apply_cmd, timeout=120)
    ...
    exit_code, stdout, stderr = client.http_exec(vmid, verify_cmd, timeout=30)
    ...
    client.http_exec(vmid, cleanup_cmd, timeout=30)
...
def main():
    ...
    parser.add_argument("--vmid", type=int, required=True, help="VMID of running container to test with")
    ...
    print(f"Testing with container: {args.vmid}")

    repo_root = Path(args.repo_root).resolve()
    asyncio.run(test_git_diff(args.vmid, cf_domain, repo_root))

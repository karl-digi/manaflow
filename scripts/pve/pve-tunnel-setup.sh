...
LEGACY (vmid-based, still supported):
...
# Legacy pattern (vmid-based): port-{port}-vm-{vmid}.${CF_DOMAIN}
#
# Examples:
...
:${CADDY_PORT} {
    # Legacy vmid-based routing
    @legacy header_regexp legacy Host ^port-(\d+)-vm-(\d+)\.
    handle @legacy {
        reverse_proxy cmux-{re.legacy.2}.${domain_suffix}:{re.legacy.1} {
...
show_status() {
    ...
    if [[ -n "${CF_DOMAIN:-}" ]]; then
        ...
        echo "Legacy URLs:  https://port-{port}-vm-{vmid}.${CF_DOMAIN}"
    fi
}

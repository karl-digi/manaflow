...
describe("buildDiffHeatmap", () => {
  it("produces tiered classes and character highlights", () => {
    const diffFile = DiffFile.createInstance({
      oldFile: { fileName: "example.ts" },
      newFile: { fileName: "example.ts" },
      hunks: [SAMPLE_DIFF],
    });
    diffFile.initRaw();
    expect(diffFile.splitLineLength).toBeGreaterThan(0);
    ...
    let newLineContent = "";
    for (let index = 0; index < diffFile.splitLineLength; index += 1) {
      const line = diffFile.getSplitRightLine(index);
      if (line?.lineNumber === 4) {
        newLineContent = (line.value ?? line.diff?.text ?? "").replace(/\r?\n$/, "");
        break;
      }
    }
    const expectedStart = Math.max(newLineContent.length - 1, 0);

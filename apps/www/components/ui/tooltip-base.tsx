...
function Tooltip({
  delayDuration = 0,
  ...props
}: React.ComponentProps<typeof BaseTooltip.Root> & { delayDuration?: number }) {
  return <BaseTooltip.Root delayDuration={delayDuration} {...props} />;
}
...
function TooltipTrigger({
  ...
  if (asChild && React.isValidElement(children)) {
    // When asChild is true, we need to merge the trigger props with the child element
    // Base UI Trigger accepts children directly, so we wrap the child
    return (
  ...
  return (
    <BaseTooltip.Portal>
      <BaseTooltip.Positioner sideOffset={sideOffset} side={side} align={align} {...props}>
        <BaseTooltip.Popup
          className={cn(
            "flex flex-col px-2 py-1 rounded-md bg-[canvas] text-sm font-sans origin-(--transform-origin) shadow-lg shadow-gray-200 outline-1 outline-gray-200 transition-[transform,scale,opacity] data-ending-style:opacity-0 data-ending-style:scale-90 data-instant:transition-none data-starting-style:opacity-0 data-starting-style:scale-90 dark:shadow-none dark:outline-neutral-600 dark:-outline-offset-1",
            className
          )}
        >
          <BaseTooltip.Arrow

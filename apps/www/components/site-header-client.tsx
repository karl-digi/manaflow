...
import { CMUX_GITHUB_REPO_URL } from "@/lib/github/constants";
...
const STAR_NUMBER_FORMATTER = new Intl.NumberFormat("en-US", {
  notation: "compact",
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="flex items-center gap-3">
          {extraEndContent}
          <GithubRepoButton stars={githubStars} />
          {showDownload ? (
            <MacDownloadLink
              autoDetect
...
function GithubRepoButton({ stars }: GithubRepoButtonProps) {
  const formattedStars = typeof stars === "number" ? formatStarCount(stars) : null;
  const label = formattedStars ?? "Star us";

  return (
    <a
      className={clsx(
        "relative hidden h-[26px] cursor-pointer items-center rounded-md border border-transparent px-2.5 py-1 text-center text-xs font-mono uppercase tracking-wide outline-none transition",
        "text-neutral-300 hover:bg-white/10 hover:text-white focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-1 focus-visible:outline-white/50 lg:flex"
      )}
      href={CMUX_GITHUB_REPO_URL}
      rel="noopener noreferrer"
      target="_blank"
      aria-label={
        formattedStars
          ? `cmux on GitHub (${formattedStars} stars)`
          : "cmux on GitHub"
      }
    >
...
function formatStarCount(stars: number) {
  ...
  return STAR_NUMBER_FORMATTER.format(stars);
}

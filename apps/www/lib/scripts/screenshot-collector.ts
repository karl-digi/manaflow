#!/usr/bin/env bun
/**
 * Screenshot Collector Script
 *
 * This script is designed to be fetched and executed remotely by the cmux worker.
 * It captures screenshots of UI changes in a PR using Claude Agent SDK.
 *
 * Usage:
 *   bun run <script> --options '<JSON options>'
 *
 * The script accepts a JSON options object with the following fields:
 *   - workspaceDir: string - The directory containing the git repository
 *   - changedFiles: string[] - List of files changed in the PR
 *   - prTitle: string - Title of the PR
 *   - prDescription: string - Description of the PR
 *   - baseBranch: string - Base branch (e.g., "main")
 *   - headBranch: string - Head branch with the changes
 *   - outputDir: string - Directory to save screenshots
 *   - auth: { taskRunJwt: string } | { anthropicApiKey: string } - Authentication
 *   - installCommand?: string - Command to install dependencies
 *   - devCommand?: string - Command to start dev server
 */
...
type InputOptions = z.infer<typeof inputOptionsSchema>;
...
function isTaskRunJwtAuth(
  auth: InputOptions["auth"],
): auth is { taskRunJwt: string } {
  return "taskRunJwt" in auth;
}

function formatClaudeMessage(message: unknown): string {
  // Type guard and simplified message formatting
  ...
  switch (type) {
    ...
    case "result": {
      const baseInfo = `${msg.num_turns} turns, ${msg.duration_ms}ms`;
      if (msg.subtype === "success") {
        return `✅ Success (${baseInfo}, $${(msg.total_cost_usd as number).toFixed(4)})`;
      }
      return `❌ Error: ${msg.subtype} (${baseInfo})`;
    }
...
async function captureScreenshotsForBranch(options: {
  workspaceDir: string;
  changedFiles: string[];
  prTitle: string;
...
- Do not create summary documents
  ...
  log(`Starting Claude Agent with browser MCP for branch: ${branch}`);
  ...
  try {
    ...
    if (useTaskRunJwt) {
      ...
      log(`Using API key auth. Key present: ${!!providedApiKey}`);
    }
    ...
    try {
      for await (const message of query({
        ...
        if (
          ...
          "structured_output" in (message as Record<string, unknown>)
        ) {
          const parsed = screenshotOutputSchema.safeParse(
            (message as Record<string, unknown>).structured_output,
          );
          if (parsed.success) {
            ...
            log(`Structured output validation failed: ${parsed.error.message}`);
          }
        }
      }
    } finally {
    ...
    log(`Failed to capture screenshots with Claude Agent: ${message}`);
...
async function captureScreenshots(
  ...
  try {
    log(`Starting PR screenshot capture in ${workspaceDir}`);
    ...
    log(`Capturing 'after' screenshots for head branch: ${headBranch}`);
    ...
    log(`Captured ${afterScreenshots.screenshots.length} 'after' screenshots`);
    ...
    const message =
      error instanceof Error ? error.message : String(error ?? "unknown error");
    log(`PR screenshot capture failed: ${message}`);
    return {
      status: "failed",
      error: message,
    };
...
async function main() {
  try {
    ...
    if (!values.options) {
      console.error("Error: --options argument is required");
      console.error('Usage: bun run <script> --options \'<JSON options>\'');
      process.exit(1);
    }
    ...
    try {
      ...
      console.error("Error: Failed to parse options JSON");
      process.exit(1);
    }

    const validationResult = inputOptionsSchema.safeParse(parsedOptions);
    ...
    const result = await captureScreenshots(validationResult.data);

    // Output the result as JSON so the caller can parse it
    console.log("\n---RESULT_JSON_START---");

...
function getConfig(): HydrateConfig {
  ...
  const prUrl = process.env.CMUX_PR_URL;
...
function checkoutPullRequest(workspacePath: string, prUrl: string) {
  log(`Checking out pull request via gh: ${prUrl}`);
  const { exitCode, stderr } = exec(`gh pr checkout "${prUrl}"`, {
    cwd: workspacePath,
    throwOnError: false,
  });
  if (exitCode !== 0) {
    log(`gh pr checkout failed: ${stderr}`, "error");
    throw new Error(`Failed to checkout pull request: ${stderr}`);
  }
...
async function main() {
  try {
    ...
    if (config.cloneUrl) {
      ...
      if (config.prUrl) {
        checkoutPullRequest(config.workspacePath, config.prUrl);
      }

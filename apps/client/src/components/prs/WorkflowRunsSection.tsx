...
function WorkflowRunsSummary({ allRuns, isLoading }: { allRuns: CombinedRun[]; isLoading: boolean }) {
  ...
  if (hasAnyRunning) {
    return (
      <div className="flex items-center gap-1 ml-2 shrink-0 text-yellow-600 dark:text-yellow-400">
        <Loader2 className="w-[10px] h-[10px] animate-pulse" />
        <span className="text-[9px] font-medium select-none">Running</span>
      </div>
    );
  }
  ...
  if (hasAnyFailure) {
    return (
      <div className="flex items-center gap-1 ml-2 shrink-0 text-red-600 dark:text-red-400">
        <X className="w-[10px] h-[10px]" />
        <span className="text-[9px] font-medium select-none">Failed</span>
      </div>
    );
  }
  ...
  if (allPassed) {
    return (
      <div className="flex items-center gap-1 ml-2 shrink-0 text-green-600 dark:text-green-400">
        <Check className="w-[10px] h-[10px]" />
        <span className="text-[9px] font-medium select-none">Passed</span>
      </div>
    );
  }

  return (
    <div className="flex items-center gap-1 ml-2 shrink-0 text-neutral-500 dark:text-neutral-400">
      <Circle className="w-[10px] h-[10px]" />
      <span className="text-[9px] font-medium select-none">Checks</span>
    </div>
  );
}
...
export function WorkflowRunsSection({
  ...
  const allPassed =
    sortedRuns.length > 0 && passedRuns.length === sortedRuns.length;
  ...
  const getStatusIcon = (status?: string, conclusion?: string) => {
    ...
    return (
      <AlertCircle className="w-3 h-3 text-neutral-500 dark:text-neutral-400" strokeWidth={2} />
    );
  };

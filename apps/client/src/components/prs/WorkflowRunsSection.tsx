...
export type CombinedRun = WorkflowRunWithMetadata;

interface WorkflowRunsOptions {
  teamSlugOrId: string;
  repoFullName?: string;
  prNumber?: number;
  headSha?: string;
...
export function useCombinedWorkflowData({
  ...
  headSha,
  limit = 50,
}: WorkflowRunsOptions): WorkflowRunsResult {
  const shouldFetch = Boolean(teamSlugOrId) && Boolean(repoFullName) && typeof prNumber === "number" && prNumber > 0;

  const queryArgs = shouldFetch
    ? {
      teamSlugOrId,
  ...
  const allRuns = useMemo<CombinedRun[]>(() => {
    ...
    for (const run of checkRuns ?? []) {
      const url =
        run.htmlUrl ||
        (queryArgs
          ? `https://github.com/${queryArgs.repoFullName}/pull/${queryArgs.prNumber}/checks?check_run_id=${run.checkRunId}`
          : undefined);
      combined.push({
        ...run,
        type: "check",
...
function getStatusDescription(run: CombinedRun) {
  ...
  if (run.type === "check" && "appSlug" in run && run.appSlug) {
    parts.push(`App: ${run.appSlug}`);
  }
  ...
  return (
    <div>
      ...
      >
        ...
        <div className="flex items-center" style={{ width: "20px" }}>
          <div className={`${summary.summaryColorClass}`}>
            {summary.summaryIcon}
          </div>
        </div>
        <span className={`text-[11px] font-semibold ${summary.summaryColorClass}`}>
          {summary.summaryText}
        </span>
      </button>
      {isExpanded && (

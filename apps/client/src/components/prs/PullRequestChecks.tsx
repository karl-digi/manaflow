...
export function useCombinedWorkflowData({
  ...
  const canQuery = trimmedRepo.length > 0 && prNumber > 0;

  const commonArgs = canQuery
    ? {
        teamSlugOrId,
  ...
  const allRuns = useMemo(
    () => {
      ...
      const workflowRunList = toArray(workflowRuns);
      const checkRunList = toArray(checkRuns);
      const deploymentList = toArray(deployments);
      const commitStatusList = toArray(commitStatuses);
      ...
      return [
        ...
        ...checkRunList.map((run) => {
          const url =
            run.htmlUrl ||
            `https://github.com/${trimmedRepo}/pull/${prNumber}/checks?check_run_id=${run.checkRunId}`;
          return {
            ...run,
            type: "check" as const,

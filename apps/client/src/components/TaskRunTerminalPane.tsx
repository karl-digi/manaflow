...
import { TaskRunTerminalSession, type TerminalConnectionState } from "./task-run-terminal-session";
...
export function TaskRunTerminalPane({ workspaceUrl }: TaskRunTerminalPaneProps) {
  ...
  const hasTerminalBackend = Boolean(xtermBaseUrl);
  ...
  const createTerminalMutation = useMutation<
    ...
    void
  >({
    mutationKey: ["terminal-tabs", workspaceUrl, xtermBaseUrl, "create"],
    ...
    onSuccess: (payload) => {
      const queryKey = terminalTabsQueryKey(xtermBaseUrl, workspaceUrl);
    ...
    onSettled: () => {
      queryClient.invalidateQueries({
        queryKey: terminalTabsQueryKey(xtermBaseUrl, workspaceUrl),
      });
    },
  });
  ...
  >({
    mutationKey: ["terminal-tabs", workspaceUrl, xtermBaseUrl, "delete"],
  ...
  // Auto-create tmux terminal on initial load if no terminals exist
  useEffect(() => {
    if (!hasTerminalBackend || hasAttemptedAutoCreate || tabsQuery.isLoading) {
      return;
    }

    if (terminalIds.length === 0 && !createTerminalMutation.isPending) {
  ...
  // Reset auto-create flag when workspace URL changes
  useEffect(() => {
  ...
  if (!xtermBaseUrl) {
    return renderMessage(
      "Waiting for workspace to expose the terminal backend..."
    );
  }
  ...
  return (
    <div className="flex flex-col h-full min-h-0">
      ...
      {createTerminalErrorMessage ? (
        <span className="text-xs text-red-500 dark:text-red-400 px-3 py-1">
          {createTerminalErrorMessage}
        </span>
      ) : null}

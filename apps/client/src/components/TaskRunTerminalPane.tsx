...
export function TaskRunTerminalPane({ workspaceUrl }: TaskRunTerminalPaneProps) {
  ...
  const createTerminalMutation = useMutation<
    ...
    void
  >({
    mutationKey: ["terminal-tabs", workspaceUrl, baseUrl, "create"],
    ...
    onSuccess: (payload) => {
      const queryKey = terminalTabsQueryKey(baseUrl, workspaceUrl);
    ...
    onSettled: () => {
      queryClient.invalidateQueries({
        queryKey: terminalTabsQueryKey(baseUrl, workspaceUrl),
      });
    },
  });
  ...
  >({
    mutationKey: ["terminal-tabs", workspaceUrl, baseUrl, "delete"],

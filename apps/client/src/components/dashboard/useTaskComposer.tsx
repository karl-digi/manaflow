...
export interface UseTaskComposerOptions {
  teamSlugOrId: string;
  initialEnvironmentId?: string;
}

export interface UseTaskComposerResult {
  ...
  onProjectSearchPaste: (input: string) => Promise<boolean>;
  ...
  selectedRepoInfo: Doc<"repos"> | null;
...
export function useTaskComposer({
...
}: UseTaskComposerOptions): UseTaskComposerResult {
  ...
  useEffect(() => {
    ...
    const val = `env:${initialEnvironmentId}`;
  ...
  }, [initialEnvironmentId, teamSlugOrId]);
  ...
  const branchOptions = branchSummary.names;
  ...
  }, [persistAgentSelection, setSelectedAgents, socket]);
  ...
  const effectiveSelectedBranch = useMemo(() => {
    ...
    const fallbackBranch = branchOptions.includes("main")
      ? "main"
      : branchOptions.includes("master")
        ? "master"
        : branchOptions[0];
    const preferredBranch =
      remoteDefaultBranch && branchOptions.includes(remoteDefaultBranch)
        ? remoteDefaultBranch
        : fallbackBranch;
    return [preferredBranch];
  }, [branchOptions, remoteDefaultBranch, selectedBranch]);

  const startTask = useCallback(async (): Promise<boolean> => {
    ...
    try {
      ...
      socket.emit(
        "start-task",
        {
          ...
          taskId,
          selectedAgents:
            selectedAgentsState.length > 0 ? selectedAgentsState : undefined,
  ...
  const selectedRepoInfo = useMemo(() => {
    ...
    return null;
  }, [reposByOrg, selectedRepoFullName]);
  ...
  const canSubmit = useMemo(() => {
    ...
    if (selectedAgentsState.length === 0) return false;
  ...
  const cloudToggleDisabled = isEnvSelected;
  ...
  return {
    ...
    selectedAgents: selectedAgentsState,

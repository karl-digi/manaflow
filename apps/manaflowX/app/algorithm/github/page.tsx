        ...
        <div className="min-w-0">
          <div className="flex items-center gap-2">
            ...
            {repo.visibility === "private" && (
              <span className="text-[10px] px-1.5 py-0.5 bg-card text-muted-foreground rounded flex-shrink-0">
                private
              </span>
            )}
          </div>
          <p className="text-xs text-muted-foreground">{formatTimeAgo(repo.lastPushedAt)}</p>
        </div>
      </div>
      <button
        onClick={onToggle}
        className={`w-10 h-5 rounded-full transition-colors relative flex-shrink-0 ${
          isMonitored ? "bg-blue-600" : "bg-muted"
        }`}
      >
        <span
      ...
      <div>
        <div className="px-4 py-3 border-b border-border">
          <div className="flex items-center justify-between">
            <h3 className="font-medium text-foreground">Repositories</h3>
            <div className="flex items-center gap-3">
              ...
              {installNewUrl && (
                <button
                  ...
                  className="flex items-center gap-1.5 px-2.5 py-1 text-sm text-foreground/80 hover:text-foreground bg-card hover:bg-accent rounded-lg transition-colors"
                >
        ...
        {repos.length === 0 ? (
          <div className="p-8 text-center">
            ...
            <p className="text-muted-foreground mb-1">No repositories connected</p>
            <p className="text-sm text-muted-foreground mb-4">Connect your GitHub account to get started</p>
            {installNewUrl && (
              <button
                onClick={handleInstallApp}
